 ...
 * @returns {string[]}
 */
export const getExprArray = (expr) => {
    // 表达式分隔为数组
 ...
 * @returns {number}
 */
export const exprArrayCalc = (exprArray) => {
    let accum = Number(exprArray[0]);
    for (let i = 0, len = exprArray.length; i < len; i++) {
        const item = exprArray[i];
        if (DELIMITERS[item]) {
            const prevIdx = i - 1;
            const nextIdx = i + 1;
            const prev = exprArray[prevIdx];
            const next = exprArray[nextIdx];
            let num1, num2;
